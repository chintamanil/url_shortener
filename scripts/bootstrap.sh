...
sudo ln -s /etc/nginx/sites-available/client.conf /etc/nginx/sites-enabled/client
#server
sudo cp -rf /vagrant/src/.nginx/server.conf /etc/nginx/sites-available/
sudo chmod 644 /etc/nginx/sites-available/server.conf
sudo ln -s /etc/nginx/sites-available/server.conf /etc/nginx/sites-enabled/server
